...
def tests(session):
    ...
    try:
        ...
        session.run(
            ...
            "--omit=src/saltext/splunk/*",
            "--include=tests/*",
        )
        try:
            session.run("coverage", "report", "--show-missing", "--include=src/saltext/splunk/*")
            ...
            #    "--include=src/saltext/splunk/*,tests/*"
            # )
        finally:
...
def gen_api_docs(session):
    ...
    session.run(
        ...
        "docs/ref/",
        "src/saltext",
        "src/saltext/splunk/config/schemas",
    )
