...
def _get_splunk(profile):
    ...
    key = "splunk_search.{}:{}:{}:{}".format(  # pylint: disable=consider-using-f-string
        config.get("host"),
        config.get("port"),
        config.get("username"),
        config.get("password"),
...
def create(name, profile="splunk", **kwargs):
    ...
    # readable by all
    config = __salt__["config.option"](profile)
    url = "https://{}:{}".format(  # pylint: disable=consider-using-f-string
    ...
    _req_url = "{}/servicesNS/{}/search/saved/searches/{}/acl".format(  # pylint: disable=consider-using-f-string
        url,
        config.get("username"),
        urllib.parse.quote(name),
    )
    requests.post(_req_url, auth=auth, verify=True, data=data)  # pylint: disable=missing-timeout
